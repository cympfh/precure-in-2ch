...
実況板においては放送毎にいくつかのスレッドが消費される.
作品ごとにいくつ消費されたかっていうのたまに話題になってたと思うんだけど、
その統計がググっても見つからなかったので、
自分で一回数えてみるかーっていう.

### 同じ文章と、用いたスクリプト
[cympfh/precure-in-2ch](https://github.com/cympfh/precure-in-2ch)
...
試しに平均値出してみよう.
一話に消費されるスレッドの数の平均値な.
初回放送と最終放送については別な効果があって除くべきなのかもしれないけど気にしない.

```bash
for f in dat/*.dat ; do echo $f; cat $f |cut -d' ' -f2 | awk -v a=0 '{a+=$1} END{print a/NR}'; done
dat/dokidoki.dat
10.0408
dat/happiness.dat
9.7551
